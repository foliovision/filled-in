...
$site_tags[0] = $_SESSION['freecap_config']['warning'];
...
$output = $_SESSION['freecap_config']['imagetype'];
...
if ($_SESSION['freecap_config']['words'] == 'random')
	$use_dict = 0;
else
	$use_dict = 1;
...
$dictionaries = glob (dirname (__FILE__).'/dictionary/*.txt');
$dict_location = $dictionaries[array_rand ($dictionaries)];

// used to calculate image width, and for non-dictionary word generation
$max_word_length = $_SESSION['freecap_config']['length'];
...
$max_attempts = $_SESSION['freecap_config']['attempts'];
...
$font_locations = glob (dirname (__FILE__).'/fonts/*.gdf');
...
$bg_images = glob (dirname (__FILE__).'/images/*.jpg');
...
if(empty($_SESSION['freecap_attempts']))
{
	...
	if($_SESSION['freecap_attempts']>$max_attempts)
	{
		...
		ImageString($im,3,15,20, $_SESSION['freecap_config']['toomany'],$red);

		sendImage($im);
	}
}

if (empty ($_SESSION['freecap_config']))
{
	$_SESSION['freecap_word_hash'] = false;
	...
	$red = ImageColorAllocate($im, 255, 0, 0);
	ImageString($im,5,15,20, "You need cookies enabled",$red);
